...
namespace CollabBuy.CollabBuyApp.View.Report
{
    public partial class AnalitikPenjualanControl : UserControl
    {
        ...
        private void LoadModeAdmin()
        {
            ...
            this.LoadAdminTable();
            this.LoadAdminChart();
            this.TampilTombolDownloadLogTxt();
        }

        private Button _btnDownloadLog;

        private void TampilTombolDownloadLogTxt()
        {
            if (_btnDownloadLog != null) return; // sudah dibuat

            _btnDownloadLog = new Button();
            _btnDownloadLog.Text = "💾 Unduh Log (.txt)";
            _btnDownloadLog.BackColor = Color.FromArgb(0, 130, 70);
            _btnDownloadLog.ForeColor = Color.White;
            _btnDownloadLog.FlatStyle = FlatStyle.Flat;
            _btnDownloadLog.FlatAppearance.BorderSize = 0;
            _btnDownloadLog.Font = new Font("Segoe UI Semibold", 9F, FontStyle.Bold);
            _btnDownloadLog.Cursor = Cursors.Hand;
            _btnDownloadLog.Size = new Size(165, 40);
            _btnDownloadLog.Location = new Point(10, 100);
            _btnDownloadLog.Click += BtnDownloadLog_Click;

            this.Controls.Add(_btnDownloadLog);
            _btnDownloadLog.BringToFront();
        }
        ...
        private void BtnDownloadLog_Click(object sender, EventArgs e)
        {
            using (SaveFileDialog sfd = new SaveFileDialog())
            {
                ...
                sfd.FileName = "Log_Aktivitas_CollabBuy_" + DateTime.Now.ToString("yyyyMMdd_HHmm") + ".txt";
                sfd.Title = "Simpan Log Aktivitas";

                if (sfd.ShowDialog() != DialogResult.OK) return;

                try
                {
                    var logs = this._adminController.GetAllActivityLogs();
                    var lines = new System.Collections.Generic.List<string>();
                    lines.Add("=============================================================");
                    lines.Add("         LOG AKTIVITAS - COLLABBUY");
                    lines.Add("=============================================================");
                    lines.Add("Diekspor pada : " + DateTime.Now.ToString("dd MMMM yyyy, HH:mm:ss"));
                    lines.Add("Total entri   : " + logs.Count);
                    lines.Add("-------------------------------------------------------------");
                    lines.Add("");

                    foreach (var log in logs)
                    {
                        lines.Add(log.DapatkanFormatLog());
                    }
                    ...
                    MessageBox.Show("Log aktivitas berhasil diunduh!\n\nFile: " + sfd.FileName,
                        "Sukses", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                catch (Exception ex)
                {
                    MessageBox.Show("Gagal mengunduh log: " + ex.Message,
